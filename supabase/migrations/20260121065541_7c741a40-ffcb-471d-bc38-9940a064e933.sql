-- Create aggregate functions for complete order analytics without row limits

-- Function to get complete country statistics
CREATE OR REPLACE FUNCTION get_country_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  country text,
  total_orders bigint,
  opt_in_count bigint,
...
-- Function to get complete city statistics with country context
CREATE OR REPLACE FUNCTION get_city_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  city text,
  country text,
  province text,
...
-- Function to get complete province statistics
CREATE OR REPLACE FUNCTION get_province_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  province text,
  total_orders bigint,
  opt_in_count bigint,
...
-- Function to get temporal statistics (by day of week and month)
CREATE OR REPLACE FUNCTION get_temporal_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  day_of_week integer,
  month_year text,
  total_orders bigint,
...
-- Function to get order value range analysis (fixed ORDER BY)
CREATE OR REPLACE FUNCTION get_order_value_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  price_range text,
  total_orders bigint,
  opt_in_count bigint
...
CREATE OR REPLACE FUNCTION get_store_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  store_id text,
  total_orders bigint,
  opt_in_count bigint,
...
-- Function to get complete summary statistics
CREATE OR REPLACE FUNCTION get_complete_summary_stats(store_filter text[] DEFAULT NULL)
RETURNS TABLE(
  total_orders bigint,
