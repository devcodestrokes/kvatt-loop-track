CREATE OR REPLACE FUNCTION public.get_customers_by_latest_order(
  ...
  opt_in_filter text DEFAULT 'all'
)
RETURNS TABLE(
  customer_id uuid,
  external_id text,
...
AS $$
  WITH matching_orders AS (
    SELECT io.customer_id, io.total_price, io.shopify_created_at, io.opt_in
    FROM imported_orders io
    WHERE io.hidden = false
      AND (
        opt_in_filter = 'all'
        OR (opt_in_filter = 'opt_in' AND io.opt_in = true)
        OR (opt_in_filter = 'opt_out' AND io.opt_in = false)
      )
  ),
  filtered_customers AS (
    ...
    WHERE (store_filter IS NULL OR ic.user_id = ANY(store_filter))
      ...
      AND (opt_in_filter = 'all' OR EXISTS (
        SELECT 1 FROM matching_orders mo WHERE mo.customer_id = ic.external_id
      ))
  ),
  total_count AS (
  ...
  order_stats AS (
    SELECT mo.customer_id AS cust_ext_id,
           count(*)::bigint AS order_count,
           coalesce(sum(mo.total_price), 0) AS total_spent,
           max(mo.shopify_created_at) AS latest_order_date
    FROM matching_orders mo
    WHERE mo.customer_id IN (SELECT p.external_id FROM paged p)
